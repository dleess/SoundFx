...
- **Play 스토어 등록정보: 입력 완료·저장됨** (short/full description, 아이콘 512px, feature graphic 1024×500, 폰 스크린샷 4장 + 7"/10" 태블릿 각 2장). **아직 Google 심사로 전송 안 함** — 콘텐츠 등급 설문·데이터 보안 설문이 남아서 앱명이 임시 표시 중. 설문 완료 후 Publishing overview에서 한 번에 전송할 것.
...
2. **Play 앱 설정 마무리**: 남은 것은 콘텐츠 등급 설문, 데이터 보안 설문("수집 안 함"), 광고 없음 선언 — 완료 후 Publishing overview에서 저장된 등록정보와 함께 Google 심사로 전송 (전송해야 임시 앱명이 정식 명칭으로 바뀜).
